...
TimingCache::TimingCache(uint32_t _numLines, CC* _cc, CacheArray* _array, ReplPolicy* _rp,
    ...
    : Cache(_numLines, _cc, _array, _rp, _accLat, _accLat, _invLat, _name), numMSHRs(mshrs), tagLat(_tagLat), ways(_ways), cands(_cands)
{
    lastFreeCycle = 0;
    lastAccCycle = 0;
    assert(numMSHRs > 0);
...
uint64_t TimingCache::access(MemReq& req) {
    ...
    if (likely(!skipAccess)) {
        ...
        printf("req.wordIdx from timing_cache.cpp: %u\n", req.wordIdx);

...
Cache::Cache(uint32_t _numLines, CC* _cc, CacheArray* _array, ReplPolicy* _rp, uint32_t _accLat, uint32_t _accSlowLat, uint32_t _accWrLat, uint32_t _accSlowWrLat, uint32_t _invLat, const g_string& _name)
    : cc(_cc), array(_array), rp(_rp), numLines(_numLines), accLat(_accLat), accSlowLat(_accSlowLat), accWrLat(_accWrLat), accSlowWrLat(_accSlowWrLat), invLat(_invLat), name(_name) {}

...
class Cache : public BaseCache {
    protected:
        ...
        uint32_t accSlowLat;
        uint32_t accWrLat;
        uint32_t accSlowWrLat;
        uint32_t invLat; //latency of an invalidation

        g_string name;

    public:
        Cache(uint32_t _numLines, CC* _cc, CacheArray* _array, ReplPolicy* _rp, uint32_t _accLat, uint32_t _accSlowLat, uint32_t _accWrLat, uint32_t _accSlowWrLat, uint32_t _invLat, const g_string& _name);

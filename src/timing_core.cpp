...
void TimingCore::PredLoadAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, BOOL pred) {
    if (pred) static_cast<TimingCore*>(cores[tid])->loadAndRecord(addr, size);
}

void TimingCore::PredStoreAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, BOOL pred) {
